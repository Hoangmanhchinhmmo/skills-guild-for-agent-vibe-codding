# 05 — Database & Data Engineering

## Database Design & Admin
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `database` | Database dev + operations | [CORE] |
| `database-design` | Schema, normalization | [CORE] |
| `database-admin` | Cloud-native DB admin | [SPEC] |
| `database-architect` | Data layer design | [SPEC] |
| `database-optimizer` | Performance optimization | [SPEC] |
| `database-migration` | Cross-ORM migrations | [SPEC] |
| `database-cloud-optimization-cost-optimize` | DB cost optimization | [SPEC] |
| `database-migrations-sql-migrations` | SQL zero-downtime | [SPEC] |
| `database-migrations-migration-observability` | Migration monitoring | [SPEC] |

## PostgreSQL
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `postgresql` | PostgreSQL schema | [CORE] |
| `postgresql-optimization` | Query tuning | [SPEC] |
| `postgres-best-practices` | Postgres optimization | [SPEC] |
| `neon-postgres` | Neon serverless Postgres | [SPEC] |
| `using-neon` | Neon best practices | [SPEC] |
| `claimable-postgres` | Temp Postgres instances | [SPEC] |

## SQL & NoSQL
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `sql-pro` | Modern SQL cloud-native | [CORE] |
| `sql-optimization-patterns` | Query optimization | [SPEC] |
| `nosql-expert` | Cassandra, MongoDB, DynamoDB | [SPEC] |
| `cc-skill-clickhouse-io` | ClickHouse analytics | [SPEC] |

## Data Engineering
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `data-engineer` | Data pipelines, warehouses | [CORE] |
| `data-engineering-data-pipeline` | Pipeline architecture | [SPEC] |
| `data-engineering-data-driven-feature` | Data-driven features | [SPEC] |
| `data-quality-frameworks` | Great Expectations | [SPEC] |
| `data-scientist` | Advanced analytics, ML | [SPEC] |
| `data-storytelling` | Data visualization narrative | [SPEC] |
| `dbt-transformation-patterns` | dbt analytics | [SPEC] |
| `spark-optimization` | Apache Spark | [SPEC] |
| `airflow-dag-patterns` | Apache Airflow DAGs | [SPEC] |

## ML & Data Science
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `ml-engineer` | Production ML systems | [SPEC] |
| `mlops-engineer` | ML pipelines, tracking | [SPEC] |
| `ml-pipeline-workflow` | MLOps E2E | [SPEC] |
| `machine-learning-ops-ml-pipeline` | ML pipeline design | [SPEC] |
| `computer-vision-expert` | YOLO, detection, segmentation | [SPEC] |

### Danh mục liên quan
- [Backend Development](02-backend.md) — API kết nối database
- [Architecture & Design](10-architecture.md) — Schema design patterns
- [Monitoring & Observability](13-monitoring.md) — Database monitoring

---

[⬅ Mobile Development](04-mobile.md) | [Quay lại Danh mục](../../README.vi.md#categories) | [DevOps & Cloud ➡](06-devops-cloud.md)

[Bắt đầu](../../guide/vi/bat-dau.md) | [Kết hợp Skills](../../guide/vi/ket-hop-skills.md) | [Cách chọn Skills](../../guide/vi/cach-chon-skills.md)
