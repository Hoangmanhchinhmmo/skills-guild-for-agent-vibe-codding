# 15 — CRM & SaaS Automation (Composio MCP)

> **Lưu ý:** Tất cả skills [AUTO] yêu cầu Rube MCP (Composio) server.
> Kiểm tra: MCP server chạy + tài khoản dịch vụ đã kết nối. Nếu không → KHÔNG thể dùng.

## CRM
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `hubspot-automation` | HubSpot CRM | [AUTO] |
| `hubspot-integration` | HubSpot integration | [AUTO] |
| `salesforce-automation` | Salesforce | [AUTO] |
| `salesforce-development` | Salesforce platform | [SPEC] |
| `pipedrive-automation` | Pipedrive | [AUTO] |
| `close-automation` | Close CRM | [AUTO] |
| `zoho-crm-automation` | Zoho CRM | [AUTO] |
| `segment-cdp` | Segment CDP | [SPEC] |

## Project Management
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `jira-automation` | Jira | [AUTO] |
| `linear-automation` | Linear | [AUTO] |
| `linear-claude-skill` | Linear issues/projects | [SPEC] |
| `linear` | Linear general | [SPEC] |
| `asana-automation` | Asana | [AUTO] |
| `clickup-automation` | ClickUp | [AUTO] |
| `monday-automation` | Monday.com | [AUTO] |
| `trello-automation` | Trello | [AUTO] |
| `wrike-automation` | Wrike | [AUTO] |
| `basecamp-automation` | Basecamp | [AUTO] |
| `todoist-automation` | Todoist | [AUTO] |

## Communication
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `slack-automation` | Slack | [AUTO] |
| `slack-bot-builder` | Slack bots (Bolt) | [SPEC] |
| `slack-gif-creator` | Slack GIFs | [SPEC] |
| `discord-automation` | Discord | [AUTO] |
| `discord-bot-architect` | Discord bots | [SPEC] |
| `microsoft-teams-automation` | MS Teams | [AUTO] |
| `telegram-automation` | Telegram | [AUTO] |
| `telegram` | Telegram Bot integration | [SPEC] |
| `telegram-bot-builder` | Telegram bots | [SPEC] |
| `telegram-mini-app` | Telegram Mini Apps | [SPEC] |
| `intercom-automation` | Intercom | [AUTO] |
| `gmail-automation` | Gmail | [AUTO] |
| `outlook-automation` | Outlook | [AUTO] |
| `outlook-calendar-automation` | Outlook Calendar | [AUTO] |

## Email Marketing
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `mailchimp-automation` | Mailchimp | [AUTO] |
| `sendgrid-automation` | SendGrid | [AUTO] |
| `activecampaign-automation` | ActiveCampaign | [AUTO] |
| `brevo-automation` | Brevo (Sendinblue) | [AUTO] |
| `convertkit-automation` | ConvertKit | [AUTO] |
| `klaviyo-automation` | Klaviyo | [AUTO] |
| `postmark-automation` | Postmark | [AUTO] |
| `email-systems` | Email systems general | [SPEC] |

## Cloud Storage
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `google-drive-automation` | Google Drive | [AUTO] |
| `google-docs-automation` | Google Docs | [AUTO] |
| `google-sheets-automation` | Google Sheets (new) | [AUTO] |
| `googlesheets-automation` | Google Sheets (legacy) | [AUTO] |
| `google-slides-automation` | Google Slides | [AUTO] |
| `google-calendar-automation` | Google Calendar | [AUTO] |
| `dropbox-automation` | Dropbox | [AUTO] |
| `box-automation` | Box | [AUTO] |
| `one-drive-automation` | OneDrive | [AUTO] |

## Dev Tools
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `github-automation` | GitHub | [AUTO] |
| `gitlab-automation` | GitLab | [AUTO] |
| `bitbucket-automation` | Bitbucket | [AUTO] |
| `circleci-automation` | CircleCI | [AUTO] |
| `vercel-automation` | Vercel | [AUTO] |
| `render-automation` | Render | [AUTO] |
| `sentry-automation` | Sentry | [AUTO] |
| `datadog-automation` | Datadog | [AUTO] |
| `pagerduty-automation` | PagerDuty | [AUTO] |

## Analytics
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `google-analytics-automation` | Google Analytics | [AUTO] |
| `amplitude-automation` | Amplitude | [AUTO] |
| `mixpanel-automation` | Mixpanel | [AUTO] |
| `posthog-automation` | PostHog | [AUTO] |
| `segment-automation` | Segment | [AUTO] |

## Calendar & Scheduling
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `cal-com-automation` | Cal.com | [AUTO] |
| `calendly-automation` | Calendly | [AUTO] |
| `zoom-automation` | Zoom | [AUTO] |

## Productivity
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `notion-automation` | Notion | [AUTO] |
| `notion-template-business` | Notion templates | [SPEC] |
| `confluence-automation` | Confluence | [AUTO] |
| `coda-automation` | Coda | [AUTO] |
| `airtable-automation` | Airtable | [AUTO] |
| `miro-automation` | Miro | [AUTO] |
| `figma-automation` | Figma | [AUTO] |
| `canva-automation` | Canva | [AUTO] |
| `notebooklm` | Google NotebookLM | [SPEC] |

## E-commerce
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `shopify-automation` | Shopify | [AUTO] |
| `shopify-apps` | Shopify apps | [SPEC] |
| `shopify-development` | Shopify dev | [SPEC] |
| `stripe-automation` | Stripe | [AUTO] |
| `square-automation` | Square | [AUTO] |
| `webflow-automation` | Webflow | [AUTO] |

## Social Media
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `twitter-automation` | Twitter/X | [AUTO] |
| `x-twitter-scraper` | X data scraping | [SPEC] |
| `x-article-publisher-skill` | X article publisher | [SPEC] |
| `linkedin-automation` | LinkedIn | [AUTO] |
| `linkedin-cli` | LinkedIn CLI | [SPEC] |
| `instagram-automation` | Instagram | [AUTO] |
| `instagram` | Instagram integration | [SPEC] |
| `tiktok-automation` | TikTok | [AUTO] |
| `youtube-automation` | YouTube | [AUTO] |
| `reddit-automation` | Reddit | [AUTO] |
| `social-orchestrator` | Social channels unified | [SPEC] |

## Customer Support
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `freshdesk-automation` | Freshdesk | [AUTO] |
| `freshservice-automation` | Freshservice | [AUTO] |
| `helpdesk-automation` | HelpDesk | [AUTO] |
| `zendesk-automation` | Zendesk | [AUTO] |
| `customer-support` | AI customer support | [SPEC] |

## HR & Legal
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `bamboohr-automation` | BambooHR | [AUTO] |
| `docusign-automation` | DocuSign | [AUTO] |

## WhatsApp
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `whatsapp-automation` | WhatsApp Business | [AUTO] |
| `whatsapp-cloud-api` | WhatsApp Cloud API | [SPEC] |
| `automate-whatsapp` | WhatsApp Kapso | [SPEC] |
| `observe-whatsapp` | WhatsApp troubleshoot | [SPEC] |

## Workflow Platforms
| Skill | Mô tả | Mức độ |
|-------|-------|--------|
| `make-automation` | Make (Integromat) | [AUTO] |
| `zapier-make-patterns` | Zapier + Make | [SPEC] |
| `n8n-code-python` | n8n Python code | [SPEC] |
| `n8n-code-javascript` | n8n JavaScript code | [SPEC] |
| `n8n-mcp-tools-expert` | n8n MCP tools | [SPEC] |
| `n8n-node-configuration` | n8n node config | [SPEC] |
| `n8n-expression-syntax` | n8n expressions | [SPEC] |
| `n8n-validation-expert` | n8n validation | [SPEC] |
| `n8n-workflow-patterns` | n8n patterns | [SPEC] |
| `workflow-automation` | Workflow automation | [SPEC] |

### Danh mục liên quan
- [SEO & Marketing](14-seo-marketing.md) — Marketing automation
- [DevOps & Cloud](06-devops-cloud.md) — Infrastructure automation
- [Workflow & Planning](19-workflow.md) — Workflow automation

---

[⬅ SEO & Marketing](14-seo-marketing.md) | [Quay lại Danh mục](../../README.vi.md#categories) | [Azure SDK ➡](16-azure-sdk.md)

[Bắt đầu](../../guide/vi/bat-dau.md) | [Kết hợp Skills](../../guide/vi/ket-hop-skills.md) | [Cách chọn Skills](../../guide/vi/cach-chon-skills.md)
